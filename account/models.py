...
class CustomUser(AbstractUser):
    ...
    def create_activation_code(self):
        """
        Метод для создания и сохранения уникального кода активации пользователя.
        """
        code = str(uuid4())
...
def password_reset_token_created(sender, instance, reset_password_token, *args, **kwargs):
    """
    Обработчик сигнала, который отправляет электронное письмо с токеном сброса пароля.

    Аргументы:
    - sender: Отправитель сигнала (класс сброса пароля).
    - instance: Инстанс сброса пароля.
    ...
    # Формируем ссылку на страницу сброса пароля, добавляя токен в качестве параметра.
